...
TEST_CASE( "Tests for 1d tsunami event.", "[TsunamiEvent1d]" ) {
  ...
  REQUIRE(l_setup.getBathymetry( 6, 0) == -30);// start of pool + displacement
  REQUIRE(l_setup.getBathymetry( 7, 0) == Approx(-30 + l_displacement * std::sin(  72 * M_PI / 180 )));
  REQUIRE(l_setup.getBathymetry( 8, 0) == Approx(-30 + l_displacement * std::sin( 144 * M_PI / 180 )));
  REQUIRE(l_setup.getBathymetry( 9, 0) == Approx(-30 - l_displacement * std::sin( 144 * M_PI / 180 )));
  REQUIRE(l_setup.getBathymetry(10, 0) == Approx(-30 - l_displacement * std::sin(  72 * M_PI / 180 )));
  REQUIRE(l_setup.getBathymetry(11, 0) == -30);// end of pool + displacement
